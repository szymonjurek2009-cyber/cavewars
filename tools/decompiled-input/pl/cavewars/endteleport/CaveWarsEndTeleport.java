...
public final class CaveWarsEndTeleport extends JavaPlugin implements Listener {
   ...
   public void onEnable() {
      ...
      this.getServer().getScheduler().runTaskTimer(this, this::tick, 1L, 1L);
      this.getLogger().info("CaveWars end teleport 2.0 enabled: final spawn return after CaveWars finishRound.");
   }
   ...
   private void tick() {
      ...
      if (var1 != null) {
         ...
         if (var1.equals("ENDING") && !this.lastState.equals("ENDING")) {
            ...
            this.getLogger().info("Round ending: captured " + this.pendingReturn.size() + " participant(s); waiting for CaveWars finishRound.");
            this.getServer().getScheduler().runTaskLater(this, () -> this.finalReturn(var2), 110L);
         }
   ...
   private void finalReturn(long var1) {
      if (var1 == this.roundSerial && !this.pendingReturn.isEmpty()) {
         ...
         if (var3 != null) {
            ...
            for (UUID var7 : new HashSet(this.pendingReturn)) {
            ...
            this.getLogger().info("Final round return: teleported " + var9 + " player(s) to main spawn; pending respawns=" + this.pendingReturn.size() + ".");
         ...
         this.getServer().getScheduler().runTaskLater(this, () -> {
            if (var1 == this.roundSerial) {
               ...
               if (!var4 && this.pendingReturn.isEmpty()) {
                  ...
                  this.participants.clear();
                  this.arenaWorldName = null;
                  this.getLogger().info("Requested CWW arena cleanup after final spawn return.");
               } else if (var3 > 0) {
   ...
   private String readGameState() {
      try {
         ...
         this.getLogger().warning("Cannot read CaveWars state: " + var9.getClass().getSimpleName() + ": " + var9.getMessage());
      }
