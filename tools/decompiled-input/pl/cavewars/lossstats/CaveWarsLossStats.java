...
import pl.cavewars.lossstats.CaveWarsLossStats.Holder;
import pl.cavewars.lossstats.CaveWarsLossStats.PlayerData;

public final class CaveWarsLossStats extends JavaPlugin implements Listener, CommandExecutor {
   ...
   public void onEnable() {
      ...
      this.store = new File(this.getDataFolder(), "losses.properties");
      this.loadOrImport();
      this.getServer().getPluginManager().registerEvents(this, this);
      PluginCommand var1 = this.getCommand("staty");
      if (var1 != null) {
         var1.setExecutor(this);
      }

      PluginCommand var2 = this.getCommand("przegrane");
      if (var2 != null) {
         var2.setExecutor(this);
      }

      this.bindGame();
      this.getServer().getScheduler().runTaskTimer(this, this::tick, 20L, 1L);
      this.getLogger().info("Loss stats enabled. /topki przegrane + /staty");
   }
   ...
   private void bindGame() {
      try {
         ...
         this.getLogger().warning("Nie mozna podpiac GameManager: " + var4.getMessage());
      }
   }
   ...
   private void tick() {
      try {
         ...
         if (var1.equals("WAITING") || var1.equals("STARTING") || var1.equals("PLAYING")) {
            for (Player var3 : this.getServer().getOnlinePlayers()) {
         ...
         this.getLogger().warning("Loss tracker tick: " + var8.getClass().getSimpleName() + ": " + var8.getMessage());
         this.game = null;
      }
   }
   ...
   private void finishIfRealRound() {
      if (this.participants.size() < 2) {
         ...
         LinkedHashSet var1 = new LinkedHashSet(this.participants);
   ...
   private void sendTop(CommandSender var1) {
      ArrayList var2 = new ArrayList(this.losses.entrySet());
   ...
   private List<PlayerData> readTopki() {
      File var1 = new File(this.getDataFolder().getParentFile(), "Topki/players.yml");
      ArrayList var2 = new ArrayList();

      try {
         List var3 = Files.readAllLines(var1.toPath(), StandardCharsets.UTF_8);
         UUID var4 = null;
         String var5 = null;
         long var6 = 0L;
         long var8 = 0L;
   ...
   private synchronized void save() {
      if (this.store != null) {
         ...
         try {
            ...
            this.getLogger().warning("Nie mozna zapisac losses.properties: " + var7.getMessage());
         }
      }
   }
